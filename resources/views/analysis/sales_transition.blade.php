    ...
    <x-slot name="header">
        ...
        <div class="flex">
            ...
            <div class="pl-2 mt-2  ml-8 ">
                <button type="button" class="w-32 text-center text-sm text-white bg-blue-500 border-0 py-1 px-2 focus:outline-none hover:bg-blue-700 rounded " onclick="location.href='{{ route('sales_transition_reset') }}'" >選択リセット</button>
            </div>
        </div>
        ...
        <div >
                <div>
                    <label for="type2" class="mr-5 leading-7 text-sm  text-gray-800 ">集計期間単位を年月・月・週から選択</label>
        ...
        <div class="md:w-2/3 sm:px-4 lg:px-4 border">
            <table class="mx-auto table-auto bg-white w-full text-center whitespace-no-wrap">
                ...
                <tr>
                    ...
                    <td class="w-3/12 pr-4 md:px-8 py-1 text-sm text-right"><span style="font-variant-numeric:tabular-nums"> {{ number_format(round($data->prev_total)/1000)}}</span></td>
                    @if($data->prev_total>0)
                    <td class="w-3/12 pr-8 md:px-8 py-1 text-sm text-right"><span style="font-variant-numeric:tabular-nums"> {{ number_format(($data->total/$data->prev_total)*100)}} %</span></td>
                    @else
