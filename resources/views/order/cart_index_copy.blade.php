<x-app-layout>
    <x-slot name="header">
        ...
        <form method="post" action="{{ route('order_confirm')}}" >
            @csrf
            <div class="px-2 w-full md:ml-10 mt-2 mb-4 flex ">
                <button type="submit" class="w-32 h-7 text-center text-sm text-white bg-pink-500 border-0 py-1 px-2 focus:outline-none hover:bg-pink-700 rounded ">登録</button>
            </div>
        <div class="md:flex">
            ...
            <div class="pl-0 mt-0">
                ...
                <div class="pl-2 ml-0 md:ml-2 w-24 h-6 text-sm items-center bg-gray-100 border rounded" name="user_id"  value="">{{ $user->id }}</div>
            </div>
            <div class="pl-2 mt-0">
                <label for="user_name" class="leading-7 text-sm  text-gray-800 dark:text-gray-200 ">Name</label>
                <div class="pl-2 w-24 h-6 text-sm items-center bg-gray-100 border rounded" name="user_name" value="">{{ $user->name }}</div>
            </div>
            <div class="pl-2 mt-0 md:mt-0 ">
                <label for="shop_id" class="leading-7 text-sm  text-gray-800 dark:text-gray-200 ">shop_ID</label>
                <div class="pl-2 w-32 h-6 text-sm items-center bg-gray-100 border rounded" name="shop_id" value="">{{ $user->shop_id }}</div>
            </div>
            <div class="pl-2 mt-0 md:mt-0 ">
                <label for="shop_name" class="leading-7 text-sm  text-gray-800 dark:text-gray-200 ">shop_Name</label>
                <div class="pl-2 w-32 h-6 text-sm items-center bg-gray-100 border rounded" name="shop_name" value="">{{ $user->shop_name }}</div>
            </div>
            </div>
            <div class="flex">
                <div class="pl-2 mt-0">
                    <label for="total_pcs" class="leading-7 text-sm  text-gray-800 dark:text-gray-200 ">total_pcs</label>
                    <div disable class="pl-2 w-24 h-6 text-sm items-center bg-gray-100 border rounded" name="total_pcs" value="">Total　 {{ $cart_total->total_pcs ?? 0}} 枚</div>
                </div>
        ...
        <div class=" mx-auto sm:px-4 lg:px-4 border ">
            <table>
                <thead>
                    <tr>
                        {{-- <th>商品ID</th> --}}
                        <th class="w-1/15 md:px-4 py-1 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100">品番ID</th>
                ...
                <tbody>
                    @foreach ($carts as $cart)
                        <tr>
                            {{-- <td>{{ $cart->sku_id }}</td> --}}
                            <td class="w-1/15 md:px-4 py-1">{{ $cart->hinban_id }}</td>
