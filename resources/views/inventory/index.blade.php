<x-app-layout>
    ...
    <form method="POST" action="{{ route('inventory_complete') }}" class="mt-2">
        ...
        <button type="submit" class="mt-4 ml-4 w-20 text-center text-sm text-white bg-pink-500 binventory-0 py-1 px-2 focus:outline-none hover:bg-pink-700 rounded " onclick="return confirm('棚卸を確定してよろしいですか？')">棚卸確定</button>
    </form>

<div class="mt-4">
    <h3 class="flex ml-4 font-semibold text-ml text-gray-800 leading-tight mt-4">
        棚卸入力明細

        <div class="flex relative ml-20">
            <label for="total_pcs" class="leading-7 text-sm text-blue-700 ">現時点棚卸総数</label>
            @if($works_total)
            <div  id="total_pcs" name="total_pcs" value="{{$works_total->pcs}}" class="h-8 text-sm w-36 text-blue-700 bg-gray-100 bg-opacity-50 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 outline-none text-gray-700 px-3 leading-8 transition-colors duration-200 ease-in-out">{{$works_total->pcs}}枚
            @else
    ...
    {{-- <table border="1" cellpadding="2" class="w-full md:w-1/2 bg-white text-center table-auto text-left whitespace-no-wrap">
        <thead>
            <tr>
                <th class="w-1/8 md:px-4 py-1">ID</th>
                <th class="w-3/8 md:px-4 py-1">入力者</th>
                <th class="w-3/8 md:px-4 py-1">商品コード</th>
                <th class="w-1/8 md:px-4 py-1 text-left">数量</th>
                <th class="w-3/8 md:px-4 py-1 text-left">　</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($works as $work)
            <tr>
                <td class="w-1/8 md:px-4 py-1">{{ $work->id }}</td>
                <td class="w-3/8 md:px-4 py-1">{{ $work->name }}</td>
                <td class="w-3/8 md:px-4 py-1">{{ substr($work->sku_id, 0, 6) . '-' . substr($work->sku_id, 6, 2) . '-' . substr($work->sku_id, 8, 1) }}</td>
                <td>
                    <form method="POST" action="{{ route('inventory_update', $work->id) }}">
                        @csrf
                        <div class="flex">
                            <input type="number" name="pcs" value="{{ $work->pcs }}" min="1" class="rounded mr-4" style="width:60px;">
