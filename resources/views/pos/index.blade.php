<x-app-layout>
    <h2 class="font-semibold text-xl mt-4 ml-4 mb-4 text-gray-800 leading-tight"> POS入力状況</h2>
    ...
    <div class="ml-4 flex mb-4 md:mb-0 md:ml-4">
        ...
        <div class="ml-4 mt-2 md:ml-4 md:mt-0">
            <button type="button" class="w-24 text-center text-sm text-white bg-indigo-500 border-0 py-1 px-2 focus:outline-none hover:bg-indigo-700 rounded " onclick="location.href='{{ route('pos_result_index') }}'" >売上Data</button>
        </div>
        <div class="ml-4 mt-2 md:ml-4 md:mt-0">
            <button type="button" class="w-24 text-center text-sm text-white bg-green-500 bpos-0 py-1 px-2 focus:outline-none hover:bg-green-600 rounded " onclick="location.href='{{ route('pos_scan') }}'" >SCAN開始</button>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('pos_complete') }}" class="mt-2">
        @csrf
        <label>メモ：<input type="text" name="memo" style="width:300px;" class="rounded w-50" ></label>
        <button type="submit" class="mt-4 ml-0 md:ml-4 w-24 text-center text-sm text-white bg-pink-500 bpos-0 py-1 px-2 focus:outline-none hover:bg-pink-700 rounded " onclick="return confirm('棚卸を確定してよろしいですか？')">確定</button>
    </form>
    </div>
    @endif


    <form method="get" action="{{ route('pos_index')}}" class="mt-4">
    <div class="flex ml-8">
    <div>
...
<div class="mt-4">
    <h3 class="flex ml-4 font-semibold text-ml text-indigo-700 leading-tight mt-4">
        ...
        <div class="flex relative ml-4">
            <label for="total_pcs" class="leading-7 text-ml text-indigo-700 ">売上数</label>
            @if($works_total)
        ...
        <div class="flex relative ml-20">
            <label for="total_gaku" class="leading-7 text-ml text-indigo-700 ">売上額</label>
            @if($works_total)
            <div  id="total_gaku" name="total_gaku" value="{{$works_total->uriage}}" class="h-8 text-ml w-24 text-indigo-700 bg-gray-100 bg-opacity-50 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 outline-none text-gray-700 px-3 leading-8 transition-colors duration-200 ease-in-out">{{$works_total->uriage}}円</div>
            @else
            <div  id="total_gaku" name="total_gaku" class="h-8 text-ml w-24 bg-gray-100 bg-opacity-50 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 outline-none text-gray-700 px-3 leading-8 transition-colors duration-200 ease-in-out"> 0 円 </div>
            @endif
        </div>
    </h3>
            ...
            <tbody>
                ...
                <tr>
                    <td class="w-5/16 md:px-2 py-1" >
                        <form method="POST" action="{{ route('pos_update', $work->id) }}">
                            @csrf
                            <div class="flex ml-1">
                                ...
                                @endif
                                <input type="number" name="pos_price2" value="{{ $work->pos_price }}"  class="rounded mr-2 text-ml " style="width:80px; font-variant-numeric:tabular-nums">
                    ...
                    <td class="w-2/16 md:px-2 py-1">
                        <div>
                        <form method="POST" action="{{ route('pos_destroy', $work->id) }}">
                            @csrf
                            @method('DELETE')
