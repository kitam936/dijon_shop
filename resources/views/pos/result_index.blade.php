<x-app-layout>
    <x-slot name="header">
        ...
        <div class="md:flex">
            ...
            <div class="pl-2 mt-2 ml-4 ">
                <button type="button" class="w-32 text-center text-sm text-white bg-indigo-500 bpos-0 py-1 px-2 focus:outline-none hover:bg-indigo-700 rounded " onclick="location.href='{{ route('pos_index') }}'" >POS入力状況</button>
            </div>
            </div>
            <div class="flex">
            ...
            <table class="md:w-2/3 bg-white table-auto w-full text-center whitespace-no-wrap">
               <thead>
                    <tr>
                        ...
                        <th class="w-2/14 md:px-4 py-1 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100">金額計</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="w-1/14 md:px-4 py-1 text-center"> <a href="{{ route('pos_result_show',['id'=>$pos_h->id]) }}" class="w-20 h-8 text-indigo-500 ml-2 "  > {{ $pos_h->id }}</a> </td>
                        <td class="w-3/14 pl-2 text-sm md:px-4 py-1 text-center"> {{ $pos_h->pos_date }}</td>
                        {{-- <td class="w-3/14 pl-2 text-sm md:px-4 py-1 text-center"> {{ $pos_h->shop_id }}</td> --}}
                        <td class="w-3/14 pl-2 text-sm md:px-4 py-1 text-center"> {{ $pos_h->shop_name }}</td>
                        <td class="w-3/14 pl-2 text-sm md:px-4 py-1 text-center"> {{ $pos_h->name }}</td>
                        <td class="w-3/14 pl-2 text-sm md:px-4 py-1 text-right"> {{ $pos_h->pcs }}</td>
                        <td class="w-3/14 pl-2 text-sm md:px-4 py-1 text-right"> {{ $pos_h->uriage }}</td>
                        @if($pos_h->status_id == 1)
                        <td class="w-3/14 pl-2 text-pink-500 text-sm md:px-4 py-1 text-center"> {{ $pos_h->status_name }}</td>
                        @else
                        <td class="w-3/14 pl-2 text-sm md:px-4 py-1 text-center"> {{ $pos_h->status_name }}</td>
                        @endif
                    </tr>
                    @endforeach
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="w-2/16 md:px-2 py-1">
                            <div>
                            <form method="POST" action="{{ route('pos_result_destroy', $pos_h->id) }}">
                                @csrf
                                @method('DELETE')
