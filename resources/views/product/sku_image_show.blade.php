<x-app-layout>
    <x-slot name="header">
        ...
        <div class="">
            ...
            <div class="p-2 w-full ml-12 flex mt-2 md:mt-0">
                <button type="button" onclick="location.href='{{ route('image_show',['hinban'=>$image->hinban_id])}}'" class="w-32 h-8 text-white text-sm bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded ">戻る</button>
            </div>
        </div>

        <div class="p-2 w-full ml-12 flex mt-0">
            @if($login_user->role_id <= 2 )
            <form id="delete_{{$image->sku_id}}" method="POST" action="{{ route('admin.sku_image_destroy',['sku'=>$image->sku_id]) }}">
                @csrf
                @method('delete')
                <div class="ml-0 mt-2 md:ml-4 md:mt-0">
                    ...
                    <a href="#" data-id="{{ $image->sku_id }}" onclick="deletePost(this)" >削除</a>
                    </div>
                </div>
            </form>
            @endif
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-2 bg-white border-b border-gray-200">
                   {{$image->hinban_id}}--{{$image->col_id}}--{{$image->size_id}}
                    <img class="w-full mx-auto" src="{{ asset('storage/public/sku_images/'.$image->filename) }}">
                </div>
            </div>
        </div>
    </div>
