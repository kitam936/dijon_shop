<x-app-layout>
    ...
    <div class="ml-2 flex mb-4 mb-4 mt-4">
        ...
        <div class="ml-4 mt-2 md:ml-4 md:mt-0">
            <button type="button" class="w-24 text-center text-sm text-white bg-indigo-500 binventory-0 py-1 px-2 focus:outline-none hover:bg-indigo-700 rounded " onclick="location.href='{{ route('move_index') }}'" >入力状況</button>
        </div>
        <div class="ml-4 mt-2 md:ml-4 md:mt-0">
    ...
    {{-- 手入力フォーム --}}
    <form method="POST" action="{{ route('move_manual') }}" class="ml-2 mt-12 md:mt-24">
        @csrf
    ...
    <h3 class="ml-4 flex font-semibold text-ml text-indigo-800 leading-tight mt-4">
        ...
        <div class="flex relative ml-20">
            <label for="total_pcs" class="leading-7 text-ml  text-indigo-800 ">合計数　</label>
            @if($works_total)
        ...
        <tbody>
            ...
            <tr>
                <td class="w-5/16 md:px-2 py-1" >
                    <form method="POST" action="{{ route('move_update2', $work->id) }}">
                        @csrf
                        <div class="flex ml-1">
                ...
                <td class="w-2/16 md:px-2 py-1">
                    <div>
                    <form method="POST" action="{{ route('move_destroy2', $work->id) }}">
                        @csrf
                        @method('DELETE')
