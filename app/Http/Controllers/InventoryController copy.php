    ...
    public function complete(Request $request)
{
    ...
    public function download_all()
    {
        ...
        $response->headers->set('Content-Disposition', 'attachment; filename="棚卸実績.csv"');
         ...
         // Status変更
         $headers=InventoryHeader::where('status_id',1)->get();
        //  dd($headers);
    ...
    public function result_index() {
        ...
        $dl_new = DB::table('inventory_headers') //未ＤＬ判定用
        ->where('inventory_headers.status_id',1)
        ->exists();

        $user = DB::table('users')
        ->where('users.id',Auth::id())->first();
            //    dd($inventory_hs,$user,$dl_new);
        return view('inventory.result_index', compact('inventory_hs','user','inventory_hs2','dl_new'));
    }
    ...
    {
        $work = InventoryHeader::findOrFail($id);
        $work->delete();
        return redirect()->route('inventory_result_index')->with(['message'=>'削除しました','status'=>'alert']);
    }
